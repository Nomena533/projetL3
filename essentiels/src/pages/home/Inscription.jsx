...
import { HiOutlineCheckCircle, HiOutlineClock, HiOutlineAcademicCap } from "react-icons/hi2";
import AnimatedSection from "../../components/AnimatedSection";
import { NIVEAUX_PARCOURS, INSTRUMENTS_DISPONIBLES, MONTANT_PAR_INSTRUMENT } from "../../lib/mockFormationData";
...
export default function Inscription() {
  ...
  const toggleInstrument = (id) =>
    setInstruments((list) => (list.includes(id) ? list.filter((x) => x !== id) : [...list, id]));

  const nombreInstruments = instruments.length;
  const montantTotal = useMemo(() => nombreInstruments * MONTANT_PAR_INSTRUMENT, [nombreInstruments]);

  const handleSubmit = (e) => {
    e.preventDefault();
    if (nombreInstruments === 0) return;
    // Intégration API à brancher ici : POST /api/inscriptions
    // (nom, prenom, email, telephone, niveau: niveau.id, instruments)
    setSent(true);
  };
  ...
  return (
    <section className="px-5 py-16 sm:px-8 sm:py-20">
      <div className="mx-auto grid max-w-6xl gap-14 lg:grid-cols-[1fr_1.3fr]">
        <AnimatedSection>
          <span className="font-mono text-xs uppercase tracking-widest text-coral-dark">Inscription</span>
          <h1 className="mt-3 font-display text-4xl font-semibold text-ink sm:text-5xl">Rejoins Kalon'ny</h1>
          <p className="mt-4 max-w-sm font-body text-ink-soft">
            Choisis le ou les instruments que tu veux apprendre. Ton parcours démarre dès validation de ton
            inscription.
          </p>

          <div className="mt-10 space-y-4 rounded-2xl border border-ivory-dark bg-white/60 p-6">
            <p className="font-mono text-[11px] uppercase tracking-wide text-coral-dark">Formation concernée</p>
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <p className="font-body text-sm font-semibold text-ink">Niveau {niveau.nom}</p>
                <p className="font-body text-xs text-ink-soft">{niveau.description}</p>
              </div>
            </div>
            <div className="flex items-center gap-3.5">
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-coral/10 text-coral-dark">
              ...
              <div>
                <p className="font-body text-sm font-semibold text-ink">Durée : {niveau.duree}</p>
                <p className="font-body text-xs text-ink-soft">{formatAriary(MONTANT_PAR_INSTRUMENT)} par instrument</p>
              </div>
            </div>
          </div>
        </AnimatedSection>
        ...
        <AnimatedSection delay={120} className="rounded-[1.75rem] border border-ivory-dark bg-white/60 p-8 sm:p-10">
          {sent ? (
            <div className="flex flex-col items-center gap-3 py-10 text-center animate-fade-in">
              ...
              <h2 className="font-display text-xl font-semibold text-ink">Inscription envoyée !</h2>
              <p className="max-w-sm font-body text-sm text-ink-soft">
                Merci {form.prenom || ""}, ton inscription au niveau {niveau.nom} pour {nombreInstruments}{" "}
                instrument{nombreInstruments > 1 ? "s" : ""} ({formatAriary(montantTotal)}) a bien été
                enregistrée. Un e-mail de confirmation arrive à {form.email || "ton adresse"}.
              </p>
              <Link to="/" className="mt-3 font-body text-sm font-semibold text-coral-dark hover:text-brick">
                Retour à l'accueil
              </Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Informations élève */}
              <div>
                <h2 className="font-display text-lg font-semibold text-ink">Tes informations</h2>
                <div className="mt-4 grid gap-5 sm:grid-cols-2">
                  <label className="block">
                    <span className="font-body text-sm font-medium text-ink">Nom</span>
                    <input
                      name="nom"
                      value={form.nom}
                  ...
                  <label className="block">
                    <span className="font-body text-sm font-medium text-ink">Prénom</span>
                    <input
                      name="prenom"
                      value={form.prenom}
                      onChange={handleChange}
                  ...
                  <label className="block">
                    <span className="font-body text-sm font-medium text-ink">Téléphone</span>
                    <input
                      type="tel"
                      name="telephone"
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-linear-to-br from-brick via-coral-dark to-coral p-5 text-ivory">
                <div>
                  ...
                  <p className="font-display text-lg font-semibold">{nombreInstruments}</p>
                </div>
                <div className="text-right">
                  <p className="font-mono text-[11px] uppercase tracking-wide text-ivory/70">Montant total</p>
                  <p className="font-display text-lg font-semibold">{formatAriary(montantTotal)}</p>
                </div>
              </div>
              ...
              >
                {nombreInstruments === 0 ? "Sélectionne au moins un instrument" : "Confirmer mon inscription"}
              </button>
            </form>
          )}
        </AnimatedSection>
