...
import { NAV_PROF, NAV_ADMIN } from "../lib/mockProfAdminData";

export default function ShellProfAmdin({ role }) {
  const [mobileOpen, setMobileOpen] = useState(false);
  const location = useLocation();
  const nav = role === "prof" ? NAV_PROF : NAV_ADMIN;
  ...
  return (
    <div className="min-h-screen bg-stone-100 font-body flex">
      ...
      >
        ...
        {/* Bascule de rôle : deux vraies routes. En production, remplace ce
            sélecteur par une redirection basée sur le rôle réel de
            l'utilisateur connecté (voir TUTORIEL.md, section 4). */}
        <div className="px-4 pt-4">
          <div className="flex bg-teal-900 rounded-sm p-1 text-xs">
            <Link to="/professeur" className={`flex-1 text-center py-1.5 rounded-sm ${role === "prof" ? "bg-amber-600 text-white" : "text-stone-300"}`}>
              Professeur
            </Link>
            <Link to="/administrateur" className={`flex-1 text-center py-1.5 rounded-sm ${role === "admin" ? "bg-amber-600 text-white" : "text-stone-300"}`}>
              Admin
            </Link>
          </div>
        </div>
