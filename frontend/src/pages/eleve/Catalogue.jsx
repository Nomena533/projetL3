...
import { Search, Filter, Music, Star } from "../../lib/icons";
import { COURSES, badge, swatch } from "../../lib/mockStudentData";
...
export default function Catalogue() {
  ...
  const filtered = COURSES.filter(
    (c) => c.titre.toLowerCase().includes(q.toLowerCase()) && (niveau === "Tous" || c.niveau === niveau)
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {filtered.map((c) => (
          <Link key={c.id} to={`/cours/${c.id}`} className="text-left bg-white border border-stone-200 rounded-sm overflow-hidden hover:border-amber-600 hover:shadow-md transition-all block">
            ...
            <div className="p-4">
              ...
              <div className="flex items-center justify-between mt-3 text-xs text-stone-500">
                <span className="flex items-center gap-1">
                  <Star size={12} className="text-amber-500 fill-amber-500" /> {c.note} ({c.avis})
                </span>
