              ...
              className={({ isActive }) =>
                `w-full flex items-center gap-3 px-3 py-2.5 rounded-sm text-sm transition-colors ${
                  isActive
                    ...
                    : "text-stone-300 hover:bg-teal-900/60"
                }`
              }
            >
              <Icon size={17} />
      ...
      <div className="flex-1 flex flex-col min-w-0 md:ml-64">
        <header className="bg-stone-50 border-b border-stone-200 px-4 md:px-8 py-4 flex items-center gap-4">
          ...
          <div className="ml-auto flex items-center gap-1.5 sm:gap-3">
            ...
            {/* ⚠️ /professeur/messages est à créer/confirmer côté routing */}
            <IconLinkBadge
            ...
            <UserMenu
              initials={initials}
              displayName={`${user?.firstname || ""} ${user?.name || ""}`.trim() || (role === "prof" ? "Professeur" : "Administrateur")}
              subLabel={
                user?.email ||
                (role === "prof" ? "Compte professeur vérifié" : "Accès administrateur")
              }
              items={[
                { to: "/professeur/profil", label: "Profil", icon: User },
                { to: "/professeur/parametres", label: "Paramètres", icon: Settings },
              ]}
